...
%         end
        ...
        ASpeed(1)=abs(Speed(2)-Speed(1))/(FixStartT(2)-FixStartT(1)-duration(1)/2+duration(2)/2);
        for i=2:lenName-1
...
%                 DScnt=DScnt+1;
%             end
            ASpeed(i)=ASpeed(i-1)+abs(Speed(i)-Speed(i+1))/(FixStartT(i+1)-FixStartT(i)-duration(i)/2+duration(i+1)/2);
...
%         Inf=Inf./max(Inf);
%         Inf=Inf+1;
%         Modulation=zeros(1,lenName-1);
%         ModulationCount=zeros(1,lenName-1);
%         DeModulation=zeros(1,lenName-1);
%         DeModulationCount=zeros(1,lenName-1);
%         for i=1:lenName-1
%             if Inf(i)>=Inf(i+1)
%                 if i==1
%                     Modulation(i)=(Inf(i)-Inf(i+1))*(scd(i)+duration(i)/2);
%                     ModulationCount(i)=1;
%                     continue;
%                 end
%                 DeModulation(i)=DeModulation(i-1);
%                 DeModulationCount(i)=DeModulationCount(i-1);
%                 Modulation(i)=Modulation(i-1)+(Inf(i)-Inf(i+1))*(scd(i)+duration(i)/2);
%                 ModulationCount(i)=ModulationCount(i-1)+1;
%                 continue;
%             end
%             if Inf(i)<Inf(i+1)
%                 if i==1
%                     DeModulation(i)=(Inf(i+1)-Inf(i))*(scd(i)+duration(i)/2);
%                     DeModulationCount(i)=1;
%                     continue;
%                 end
%                 Modulation(i)=Modulation(i-1);
%                 ModulationCount(i)=ModulationCount(i-1);
%                 DeModulation(i)=DeModulation(i-1)+(Inf(i+1)-Inf(i))*(scd(i)+duration(i)/2);
%                 DeModulationCount(i)=DeModulationCount(i-1)+1;
%                 continue;
%             end
%         end
...
%             D(i)=acosd((x1*x2+y1*y2+z1*z2)/((x1^2+y1^2+z1^2)^0.5*(x2^2+y2^2+z2^2)^0.5));
            if Inf(i)-Inf(i+1)>=0
                D(i)=abs(Inf(i)-Inf(i+1));%*(scd(i)+duration(i)/2);
            end
            if Inf(i)-Inf(i+1)<0
                D(i)=1/abs(Inf(i)-Inf(i+1));%*(scd(i)+duration(i)/2);
            end
%             D(i)=D(i)/scd(i);
        end
%         D=D/max(D)+1;
...
%             end
%             if Inf(t+1)-Inf(t)<0
%                 dt(NewName(t,1),NewName(t+1,1),t)=(Inf(t)-Inf(t+1));
%             end
            if t==1
                TranMatrix(NewName(t,1),NewName(t+1,1),t)=D(i);
...
%             for i=1:len
%                 for j=1:len
%                     TranMatrix(i,j,t)=InfChange(i,t)*InfChange(j,t);
%                 end
%             end
%         end
%         for t=1:lenName-1
%             for i=1:len
%                 for j=1:len
%                     TranMatrixCount(i,j,t)=InfCount(i,t)*InfCount(j,t);
%                 end
%             end
%         end
%         for t=1:lenName-1
...
%         aSGED=sum(abs(HS-H))/lenName./max(H);
        DataM(pNo,:,sNo)=[TInfJSD(stop+1)/TEJSD(stop),InfJSD(stop+1)/PEJSD(stop),TIO(stop),TEJSD(stop),TInfJSD(stop+1),PEJSD(stop),InfJSD(stop+1),Hij(stop),HS(stop),H(stop),ESRate,ERate,FixRate,stop/ASpeed(stop)];%mean(AcceleratedSpeed)
    end
end
