...
for sNo=1:4
    ...
    for pNo=3:14
        ...
        Vec=xlsread(FileName,'AI:AM');
        len=floor(1*size(Vec,1));

        EyeTheta=Vec(1:len,1);
        EyePhi=Vec(1:len,2);
%         HeadTheta=Vec(1:len,4);
%         HeadPhi=Vec(1:len,5);
        
        HeadVec=xlsread(FileName,'V:X');
        HeadTheta=90-atand( HeadVec(:,2)./HeadVec(:,3) );
...
%         HeadPhi=HeadPhi./max(HeadPhi);
        ...
        HeadState=[PupilDila];%HeadPhi,HeadTheta
        ...
        for st=1:SurrogateTimes
            SE=Surrogate(EyeState);
            SH=Surrogate(HeadState);
